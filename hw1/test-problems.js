...
async function run() {
  ...
  const loaderMod = tryRequire('./sm3191/loadSubjects') || tryRequire('./solutions/loadSubjects');
  let subjects = null;
  if (loaderMod) {
  ...
  for (const problem of toRun) {
    ...
    const otherPath = preferredPath.startsWith('./sm3191') ? './solutions/' + problem.replace('p', 'problem') : './sm3191/' + problem.replace('p', 'problem');
