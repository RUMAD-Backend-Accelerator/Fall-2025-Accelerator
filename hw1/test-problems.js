...
async function run() {
  ...
  const loaderMod = tryRequire('./bx83/loadSubjects') || tryRequire('./solutions/loadSubjects');
  let subjects = null;
  if (loaderMod) {
  ...
  for (const problem of toRun) {
    ...
    const otherPath = preferredPath.startsWith('./bx83') ? './solutions/' + problem.replace('p', 'problem') : './bx83/' + problem.replace('p', 'problem');
